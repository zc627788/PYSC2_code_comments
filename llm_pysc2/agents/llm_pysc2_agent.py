...
class LLMAgent:
  ...
  def update(self, obs):
    ...
    if self.name not in self.config.AGENTS_ALWAYS_DISABLE and self.enable:
      ...
      if not os.path.exists(self.log_dir_path + f"/{self.name}"):
        os.makedirs(self.log_dir_path + f"/{self.name}", exist_ok=True)
  ...
  def get_text_o(self, obs) -> str:
    ...
    # 使用观察转换器进行转换
    # 这里会调用CombatGroupTranslatorO.translate()等方法
  ...
  def get_func(self, obs):
    ...
    if len(self.func_list) == 0:
        # 步骤2: 从action_list中取出一个动作
      action = self.action_list.pop(0)
         # 步骤3: 处理动作（添加工人选择、训练研究等）
