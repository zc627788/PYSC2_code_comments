...
class MainAgent(base_agent.BaseAgent):
  ...
  def _initialize_logger(self):
    ...
    while True:
      self.log_id += 1
      # 构建日志目录路径，包含地图和种族信息
      # if hasattr(self.config, 'map_name') and hasattr(self.config, 'agent_race'):
      #   self.log_dir_path = f"{os.path.dirname(os.path.abspath(__file__))}/../../llm_log/{self.config.agent_race}_{self.config.map_name}_{self.start_time}-{self.log_id}"
      # else:
  ...
  def step(self, obs):
    """
    主智能体的核心步骤函数 - 游戏观察阶段的核心控制器
    
    功能：
    - 接收PySC2游戏观察对象
    - 执行各种初始化步骤（摄像头校准、单位分组等）
    - 管理子智能体的观察收集和LLM查询流程
    - 协调多智能体的协作和通信
    - 返回游戏动作给PySC2环境
    
    Args:
        obs: PySC2游戏观察对象，包含当前游戏状态
    
    Returns:
        FunctionCall: PySC2可执行的游戏动作
    """
    super(MainAgent, self).step(obs)

    # ==================== 主智能体控制数据更新 ====================
    agent_name = None
    self.obs_history.append(obs)  # 记录观察历史
    self.data_recorder.step(obs, self.episodes, self.steps)  # 记录游戏数据
    
    # 动态调整摄像头和选择阈值
    if len(self.func_id_history) > 0 and self.func_id_history[-1] == 573:
      # 如果上次执行了摄像头移动，增加摄像头阈值（避免频繁移动）
      self.camera_threshold += 0.05
    elif len(self.func_id_history) > 0 and self.func_id_history[-1] == 3:
      # 如果上次执行了矩形选择，增加选择阈值（提高选择成功率）
    ...
    if self.main_loop_step_old != self.main_loop_step:
      ...
      self.main_loop_lock = False  # 解锁主循环
    ...
    # 步骤2: 单位分组，添加到相关智能体团队 (必要步骤)
    # 功能：将游戏中的单位分配给不同的智能体团队
    ...
    for agent_name in self.AGENT_NAMES:
      agent = self.agents[agent_name]
      agent.num_step = self.steps                    # 游戏步数
      agent.main_loop_step = self.main_loop_step     # 主循环步数
      agent.world_range = self.world_range           # 世界范围
      agent.world_x_offset = self.world_x_offset     # 世界X偏移
      agent.world_y_offset = self.world_y_offset     # 世界Y偏移
      agent.size_screen = self.size_screen           # 屏幕大小
      agent.size_minimap = self.size_minimap         # 小地图大小
