...
class CombatGroupTranslatorO(BaseTranslatorO):
  """
  战斗组观察转换器 - 游戏观察阶段的核心组件
  
  功能：
  - 将PySC2游戏观察转换为战斗组智能体可理解的文本描述
  ...
  def translate(self, agent) -> str:
    ...
    - 收集所有团队的观察数据
    - 提取游戏基础信息（时间、资源、科技等）
    - 获取单位详细信息（位置、血量、能量、技能等）
    - 列出当前可执行的有效动作
    - 包含任务目标和通信信息
    
    Args:
        agent: LLM智能体对象，包含所有观察数据
    
    Returns:
        str: 完整的文本观察描述，供LLM决策使用
    """
